...
void AnnotatedCameraWidget::drawHud(QPainter &p) {
  ...
  if (this->headLessMode) {
    ...
    max_speed_headless_rect.setRect(120 * 1.5, common_y_headless, max_speed_headless_size.width(), max_speed_headless_size.height());
  ...
  if (!(bigMapOpen || hideSpeed)) {
    if (standstillDuration > 1) {
      ...
      if (this->headLessMode) {
        ...
        drawText(p, rect().center().width() * 0.7, 210 * 0.8, speedStr);
        p.setFont(InterFont(66));
        drawText(p, rect().center().width() * 0.7, 290 * 0.8, speedUnit, 200);
