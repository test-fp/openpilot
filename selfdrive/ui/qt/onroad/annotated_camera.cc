...
AnnotatedCameraWidget::AnnotatedCameraWidget(VisionStreamType type, QWidget* parent) : fps_filter(UI_FREQ, 3, 1. / UI_FREQ), CameraWidget("camerad", type, true, parent) {
  ...
  // Create and add distance_btn here, BEFORE other widgets are added to main_layout
  distance_btn = new DistanceButton(this);
  buttons_layout->addWidget(distance_btn, 0, Qt::AlignBottom | Qt::AlignLeft);
  // == END MODIFICATION FOR APPROACH 2 ==
  //}
...
void AnnotatedCameraWidget::drawHud(QPainter &p) {
  ...
  if (this->headLessMode) {
    ...
    max_speed_headless_rect.setRect(rect().width() * 0.35, common_y_headless, max_speed_headless_size.width(), max_speed_headless_size.height());
    ...
    if (has_us_speed_limit || has_eu_speed_limit) { //
      ...
      int speed_limit_x_headless = max_speed_headless_rect.right() + (rect().width() * 0.05); // + element_spacing_headless ;
      ...
      if (has_us_speed_limit) { //
        ...
        drawRoundedRect(p, speed_limit_headless_rect, 24, 24 ,24 ,24); //
        //drawRoundedRect(speed_limit_headless_rect, 24, 24); //
        p.setPen(QPen(blackColor(), 6)); //
        drawRoundedRect(p, speed_limit_headless_rect.adjusted(9, 9, -9, -9), 16, 16, 16, 16); //
  ...
  if (!(bigMapOpen || hideSpeed)) {
    if (standstillDuration > 1) {
      ...
      if (this->headLessMode) {
        ...
        drawText(p, rect().width() * 0.3, 210 * 0.8, speedStr);
        p.setFont(InterFont(66));
        drawText(p, rect().width() * 0.3, 290 * 0.8, speedUnit, 200);
